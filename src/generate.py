...
def write_output(parsed: dict[str, Page], output_path: str) -> None:
    ...
    for relative_path, page in parsed.items():
        ...
        title = page.meta.get('title', os.path.splitext(os.path.basename(relative_path))[0])

        html = LAYOUT.format(title=title, content=page.render(), css_path=css_relative_path)
