...
export class Parser {
    ...
    parse() {
        ...
        while (!this.isAtEnd()) {
            ...
            while (this.match(TokenType.NEWLINE)) {} // Skip extra newlines
        }
    ...
    parseForStatement() {
        const identifier = this.consume(TokenType.IDENTIFIER, "Expected variable name in for loop").value;
    ...
    parseExpressionOrAssignment() {
        ...
        if (this.matchOperator(':=') || this.matchOperator('=')) {
            ...
            if (expr.type === 'Identifier') {
                ...
                return { type: 'ArrayAssignmentStatement', identifier: expr.identifier, index: expr.index, value };
            } else {
    ...
    parseCallOrAccess() {
        ...
        while (true) {
            if (this.matchPunctuation('(')) {
                ...
                if (expr.type !== 'Identifier') {
                     throw this.error(this.previous(), "Can only access array elements on identifiers currently");
                }
