    ...
    if (node.type === 'Program' || node.type === 'Block') {
      ...
      try {
        switch (stmt.type) {
          ...
          case 'WhileStatement': {
            ...
            if (cond) {
              frame.index--;
              stack.push({
                node: { type: 'Block', body: stmt.body },
                index: 0,
                isLoop: true,
                scope: {},
          ...
          case 'ForToStatement': {
            if (frame.forInitialized === undefined) {
              ...
              frame.forVarName = stmt.identifier;

              if (frame.scope.hasOwnProperty(stmt.identifier)) {
                frame.forOriginalValue = frame.scope[stmt.identifier];
                frame.forHadOriginalValue = true;
              } else {
                frame.forHadOriginalValue = false;
              }

              frame.scope[stmt.identifier] = startVal;
              ...
              if (startVal <= endVal) {
                ...
                if (frame.forHadOriginalValue) {
                  frame.scope[stmt.identifier] = frame.forOriginalValue;
                } else {
                  delete frame.scope[stmt.identifier];
                }
                frame.forInitialized = undefined;
                return createNextState(functions, stack, stmt);
              }
            } else {
              const cur = frame.scope[stmt.identifier];
              const nextVal = cur + 1;

              if (nextVal <= frame.forEndVal) {
                frame.scope[stmt.identifier] = nextVal;
                frame.index--;
                stack.push({
                  node: { type: 'Block', body: stmt.body },
                  index: 0,
            ...
            while (stack.length > 0) {
              ...
              if (popped.isLoop) {
                if (stack.length > 0) {
                  ...
                  parent.index++;
                  if (parent.forInitialized !== undefined) {
                    if (parent.forHadOriginalValue) {
                      parent.scope[parent.forVarName] = parent.forOriginalValue;
                    } else {
                      delete parent.scope[parent.forVarName];
                    }
        ...
        if (e instanceof FunctionCallSuspend) {
          ...
          stack.push({
            node: { type: 'Block', body: e.funcDef.body },
            index: 0,
            scope: e.localVars,
            isFunction: true,
          });
